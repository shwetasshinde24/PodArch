 ...
 * objcopy uses it
 */
char *tempintsection_file = "./podint";
...
static unsigned char* elf_write_binary_podintc(struct signelf_info *sinfo){
...
//	file_sz = i_size_read(file->f_path.dentry->d_inode);
	...
	if (!found_podid_section) {
		...
		retval = 0;
		printf("Not found pod_inc\n");
		goto out_free_shtable;
	}
	else
		printf("Found pod_inc\n");
...
//	sig_info_sz = sizeof(struct elf_sig_info);
	...
	if(int_len!=sinfo->integrity_lenc){
	...
	write_len = pwrite(fd, sinfo->integrityc,sinfo->integrity_lenc, offset);
	if (write_len == -1) {
...
static unsigned char* elf_write_binary_podintd(struct signelf_info *sinfo){
...
//	file_sz = i_size_read(file->f_path.dentry->d_inode);
	...
	/* Scan for section with name ".pod_ind" */
	for (i = 0; i < ehdr->e_shnum; i++) {
		char sec_name[9];
		offset = elf_shstrpnt->sh_offset + elf_spnt->sh_name;
		retval = pread(fd, sec_name, 9, offset);
	...
	if (!found_podid_section) {
		/* File is not pod-enabled */
		retval = 0;
		printf("Not found pod_ind\n");
		goto out_free_shtable;
	}
	else
		printf("Found pod_ind\n");
...
//	sig_info_sz = sizeof(struct elf_sig_info);
	...
	int_len = elf_spnt->sh_size;
	if(int_len!=sinfo->integrity_lend){
		printf("int_len!=sinfo->integrity_len\n");
		goto out_free_shtable;
	}
	...
	write_len = pwrite(fd, sinfo->integrityd,sinfo->integrity_lend, offset);
	if (write_len == -1) {
		fprintf(stdout, "Failed to write:%s\n",
				strerror(errno));
		printf("Error\n");
...
//	unsigned int currentbufsize = 0;
	...
	for (i = 0; i < sinfo->ehdr.e_phnum, first_segment!=0; i++) {
		...
		while (sz_rem)  {
			...
			if (buf_len == 0)
				break;
//			if(((page_count*4096)+base)!=0x446000 && ((page_count*4096)+base)!=0x40e000 && ((page_count*4096)+base)!=0x448000){ // && ((page_count*4096)+base)!=0x40e000){ */
...
//			}
		...
		printf("page_count=%d, intsize=%d\n",page_count, currentintsize);
		if (sz_done != sz) {
...
//	unsigned int currentbufsize = 0;
	...
	for (i = 0; i < sinfo->ehdr.e_phnum; i++) {
		...
		while (sz_rem)  {
			...
			if (buf_len == 0)
				break;
			//		if(((page_count*4096)+base)!=0x495000 && ((page_count*4096)+base)!=0x4b4000 && ((page_count*4096)+base)!=0x497000){ // && ((page_count*4096)+base)!=0x40e000){ */
//			if(((page_count*4096)+base)!=0x4b1000 && ((page_count*4096)+base)!=0x4dd000 && ((page_count*4096)+base)!=0x4b3000 && ((page_count*4096)+base)!=0x4de000 && ((page_count*4096)+base)!=0x4b2000 && ((page_count*4096)+base)!=0x4b4000){
			CT = (unsigned char*)malloc(PTLEN);	
			memset(CT, 0, PTLEN);
			EVP_CIPHER_CTX *ctx = EVP_CIPHER_CTX_new();
...
static int sign_elf_executable(struct signelf_info *sinfo)
{
	...
	if (encrypt_and_hash_elfc(sinfo)) {
		ret = 1;
	 	fprintf(stdout, "Error while encrypting elf code\n");
		goto out;
	}
	/* if (encrypt_and_hash_elfd(sinfo)) { */
	/* 	ret = 1; */
	/*  	fprintf(stdout, "Error while encrypting elf data\n"); */
	/* 	goto out; */
	/* } */
	...
	if(elf_write_binary_podintc(sinfo)==NULL){
		fprintf(stdout, "Error while writing pod_int into the elf\n");
		goto out;
	}
		/* if(elf_write_binary_podintd(sinfo)==NULL){ */
	/* 	fprintf(stdout, "Error while writing pod_int into the elf\n"); */
	/* 	goto out; */
	/* } */
