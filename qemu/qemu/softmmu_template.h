...
//				printf("Found a pod-enabled app:%" PRIu64 ", Reg_kapp=%lld\n", ccurrent->key, Reg_kapp);
				...
				if(ccurrent->key==Reg_kapp){ /* check the current executing authority */
					...
					HASH_ITER(ahh, pairs, p, tmp1) {
						if((mask_addr>p->start && mask_addr<p->end)){
							printf("Found addr_pair_t: mask_addr=0x%x, s=0x%x,e=0x%x\n", addr & POD_PAGE_MASK, p->start, p->end);
							skip=true;
						}
						else printf("Not found addr_pair_t: mask_addr=0x%x, s=0x%x,e=0x%x\n", addr & POD_PAGE_MASK, p->start, p->end);
					}
					if(skip){
						...
						printf("Found mask_addr: 0x%x, s=0x%x, e=0x%x\n", addr & POD_PAGE_MASK, start_code_address, end_code_address);
						//if(skip){
						IPACT_struct *item;
...
//							printf("Calling get_int_addr1\n");
							target_ulong pod_int_address = get_int_address(addr,env);
							int pod_mmu_idx = get_mmuidx(addr,env);
...
//				printf("Encryption starts with KEY:*%s*\n",tempkeybuf);
	...
	out:	
        ...
        if ((addr & (DATA_SIZE - 1)) != 0)
			...
			VDF_time:
		if(Reg_kapp){
			printf("Inside VDF_time, Calling get_int_address\n");
			target_ulong pod_int_address = get_int_address(addr,env);
