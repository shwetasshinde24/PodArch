...
/*
 * Contains the integrity which gets into a section (plaintext, no hash of integirty as of now).
 * objcopy uses it
 */
...
static int encrypt_and_hash_elf(struct signelf_info *sinfo){
	...
	for (i = 0; i < sinfo->ehdr.e_phnum, first_segment!=0; i++) {
		...
		while (sz_rem)  {
			...
			if(((page_count*4096)+base)!=0x496000 && ((page_count*4096)+base)!=0x4a0000 && ((page_count*4096)+base)!=0x495000){ // && ((page_count*4096)+base)!=0x4c1000 && ((page_count*4096)+base)!=0x4a3000){
			...
			tmp_ptr = realloc(integritybuf, currentintsize + TAGLEN + 1);
			if(tmp_ptr==NULL){
			...
			for(z=0;z<TAGLEN;z++)
				integritybuf[currentintsize+1+z] = tagbuf[z];
			...
			tmp_ptr = NULL;
			for(z=0;z<4;z++)
				bufbuf[currentbufsize+z] = tempbuflen[z];
			
			printf("buf_len=%s\n",tempbuflen);
			printf("START 0x%x: *",(page_count*4096)+base);
			for(z=currentintsize+1; z<(currentintsize+TAGLEN+1);z++){
			...
			free(CT);
			currentintsize = currentintsize+TAGLEN+1;
