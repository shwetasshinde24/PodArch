...
#define POD_PAGESTART(_v) ((_v) & ~(unsigned long)(POD_PAGESIZE-1))
#define POD_PAGEOFFSET(_v) ((_v) & (POD_PAGESIZE-1))
#define POD_PAGEALIGN(_v) (((_v) + POD_PAGESIZE - 1) & ~(POD_PAGESIZE - 1))
...
static int check_pod_header(struct pod_hdr *pod_ex)
{
	...
	if (pod_ex->system_id != POD_SID_PARISC_1_0 &&
	    pod_ex->system_id != POD_SID_PARISC_1_1 &&
	    pod_ex->system_id != POD_SID_PARISC_2_0)
		return -ENOEXEC;

	if (pod_ex->a_magic != POD_EXEC_NONSHARE &&
	    pod_ex->a_magic != POD_EXEC_SHARE &&
	    pod_ex->a_magic != POD_EXEC_DEMAND)
		return -ENOEXEC;

	if (pod_ex->version_id != POD_ID_OLD &&
	    pod_ex->version_id != POD_ID_NEW)
		return -ENOEXEC;
...
static int map_pod_binary(struct file *file,
		const struct pod_exec_auxhdr *hpuxhdr)
{
	unsigned long code_start, code_size, data_start, data_size;
	unsigned long bss_start, pod_brk;
	int retval;
	...
	code_start = POD_PAGESTART(hpuxhdr->exec_tmem);
	code_size = POD_PAGEALIGN(hpuxhdr->exec_tsize);
	current->mm->start_code = code_start;
	current->mm->end_code = code_start + code_size;
	down_write(&current->mm->mmap_sem);
	retval = do_mmap(file, code_start, code_size, prot,
			flags, POD_PAGESTART(hpuxhdr->exec_tfile));
	up_write(&current->mm->mmap_sem);
	if (retval < 0 && retval > -1024)
		goto out;

	data_start = POD_PAGESTART(hpuxhdr->exec_dmem);
	data_size = POD_PAGEALIGN(hpuxhdr->exec_dsize);
	current->mm->start_data = data_start;
	current->mm->end_data = bss_start = data_start + data_size;
	down_write(&current->mm->mmap_sem);
	retval = do_mmap(file, data_start, data_size,
			prot | PROT_WRITE, flags,
			POD_PAGESTART(hpuxhdr->exec_dfile));
	up_write(&current->mm->mmap_sem);
	if (retval < 0 && retval > -1024)
		goto out;

	pod_brk = bss_start + POD_PAGEALIGN(hpuxhdr->exec_bsize);
	current->mm->start_brk = current->mm->brk = pod_brk;
	down_write(&current->mm->mmap_sem);
	retval = do_mmap(NULL, bss_start, pod_brk - bss_start,
...
load_pod_binary(struct linux_binprm * bprm, struct pt_regs * regs)
{
	...
	struct pod_hdr *pod_ex;
	struct pod_exec_auxhdr *hpuxhdr;

	/* Get the exec-header */
	pod_ex = (struct pod_hdr *) bprm->buf;

	retval = check_pod_header(pod_ex);
	if (retval != 0)
		goto out;
	...
	retval = -ENOMEM;
	size = pod_ex->aux_header_size;
	if (size > POD_PAGESIZE)
		goto out;
	hpuxhdr = kmalloc(size, GFP_KERNEL);
	if (!hpuxhdr)
		goto out;

	retval = kernel_read(bprm->file, pod_ex->aux_header_location,
			(char *) hpuxhdr, size);
	if (retval != size) {
		if (retval >= 0)
			retval = -EIO;
	...
	retval = map_pod_binary(bprm->file, hpuxhdr);
	if (retval < 0)
		goto out_free;

	pod_entry = hpuxhdr->exec_entry;
	kfree(hpuxhdr);
	...
	start_thread_pod(regs, pod_entry, bprm->p);
	return 0;
